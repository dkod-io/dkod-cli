//! Cursor agent CLI capture adapter.
//!
//! - [`parse_events`] reads a Cursor agent NDJSON transcript and maps its
//!   records onto our [`crate::Session`] / [`crate::Message`] model.
//! - [`capture_cursor`] spawns `cursor-agent -p --output-format stream-json`,
//!   parses the NDJSON stream, and builds a Session.
//!
//! Cursor's stream-json format mirrors Claude Code's structure: typed
//! events with assistant messages, tool use, and tool results.
...
/// Options for [`capture_cursor`].
#[derive(Debug, Clone)]
pub struct CaptureOptions {
    /// User args forwarded after `cursor-agent -p --output-format stream-json`.
    pub args: Vec<String>,
    /// Resolved Cursor agent binary path (`$DKOD_CURSOR_BIN` or `cursor-agent`).
    pub cursor_bin: PathBuf,
    /// Working directory.
    pub cwd: PathBuf,
}

/// Pure parser: read a Cursor NDJSON transcript and map to a [`Session`].
pub fn parse_events(events_path: &Path) -> Result<Session> {
    ...
    for line in reader.lines() {
        let line = match line {
            Ok(l) => l,
            Err(e) => {
                eprintln!("dkod: cursor: read error: {e}");
                continue;
            }
        };
        ...
        match serde_json::from_str::<serde_json::Value>(&line) {
            Ok(v) => events.push(v),
            Err(e) => eprintln!("dkod: cursor: skipping malformed JSON: {e}"),
        }
    }
    parse_event_records(&events)
}

/// Production wrapper: spawn `cursor-agent -p --output-format stream-json`
/// and parse the NDJSON stream from stdout.
pub fn capture_cursor(opts: CaptureOptions) -> Result<Session> {
    let mut cmd = Command::new(&opts.cursor_bin);
    ...
    let mut child = cmd.spawn().with_context(|| {
        format!(
            "spawn {} -p --output-format stream-json (is cursor-agent installed?)",
            opts.cursor_bin.display()
        )
    })?;
    ...
    let stdout = child
        .stdout
        .take()
        .ok_or_else(|| anyhow!("cursor-agent stdout was not captured"))?;
    ...
    for line in reader.lines() {
        let line = match line {
            Ok(l) => l,
            Err(e) => {
                eprintln!("dkod: cursor stdout read error: {e}");
                continue;
            }
        };
        if line.trim().is_empty() {
        ...
        match serde_json::from_str::<serde_json::Value>(&line) {
            Ok(v) => events.push(v),
            Err(e) => eprintln!("dkod: cursor stdout: skipping non-JSON line: {e}"),
        }
    }

    let status = child.wait().context("wait for cursor-agent child")?;
...
mod tests {
    ...
    fn fixture_path() -> PathBuf {
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("testdata/cursor/synthetic-events.jsonl")
    }

    #[test]
    fn parse_events_synthetic_fixture() {
        let session = parse_events(&fixture_path()).expect("parse events");
        assert!(matches!(session.agent, crate::Agent::Cursor));
