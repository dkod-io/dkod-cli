...
pub fn run(cwd: &Path, args: Vec<String>) -> Result<()> {
    ...
    let cursor_bin: PathBuf = std::env::var_os("DKOD_CURSOR_BIN")
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("cursor-agent"));

    let mut session =
        dkod_core::capture::cursor::capture_cursor(dkod_core::capture::cursor::CaptureOptions {
            args,
            cursor_bin,
            cwd: cwd.to_path_buf(),
        })
        .context("capture cursor session")?;
